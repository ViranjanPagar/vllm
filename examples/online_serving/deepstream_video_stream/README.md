...
GStreamer, plus a streaming endpoint for live RTSP captioning.
...
| `test_rtsp_stream.py` | RTSP smoke test |
...
### Smoke test live RTSP captioning

```bash
python3 vllm/examples/online_serving/deepstream_video_stream/test_rtsp_stream.py \
    rtsp://10.24.217.130:8554/ --chunk-duration 5
```

Replace the URI with your own RTSP source. Each `--chunk-duration` seconds
of video are decoded and sent to the model as one chat completion request.
...
HTTP request (OpenAI /v1/chat/completions)
    │
    │  video_url: file:///data/video/...  or  rtsp://...
    │
    ▼
...
For live RTSP, see also:

- `vllm/multimodal/rtsp_stream_manager.py` — per-URI worker + subscribe/unsubscribe
- `vllm/entrypoints/openai/streaming/` — HTTP endpoints for streaming captions
...
- **RTSP `500 Internal Server Error`**
  Usually the URI lacks a mount-point or UDP RTP is blocked by Docker
  bridge networking. Try `--network=host`, and force TCP transport at
  the GStreamer level if your code wires `rtspsrc protocols=tcp`.
